...
class MegatronPolicyWorker(AbstractPolicyWorker, ColocatablePolicyInterface):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if self.cfg.get("hybrid_cp", {}).get("enabled", False):
            if "megatron_cfg" not in self.cfg:
                self.cfg["megatron_cfg"] = {}
            self.cfg["megatron_cfg"]["hybrid_context_parallel"] = True
            logger.info(
                "HCP enabled: hybrid_context_parallel=True will be passed to Megatron initialization"
            )
